...
const QuoteListHeader = ({ productCount, totalUnits, isDark, onClear }) => {
  ...
  return (
    <div className="mb-8 flex flex-col gap-5">
      <Helmet>
        <title>My Quotes | Onasis Links Resources Limited</title>
...
const QuoteItemCard = ({ item, isDark, onRemove, onUpdateQuantity }) => {
  ...
  return (
    <article
      className={
        isDark
          ? "rounded-2xl border border-[#2A2A2A] bg-[#1A1A1A] p-4 shadow-sm shadow-black/10 transition-colors sm:p-5"
          ...
          <div className="flex items-start justify-between gap-3">
            <div className="min-w-0 flex-1">
              <div className="mb-2 flex flex-wrap items-center gap-2">
                {category && (
                  <span
                    className={
                      isDark
                        ? "inline-flex rounded-full border border-[#2A2A2A] bg-[#0a0a0a] px-2 py-0.5 text-[10px] font-medium uppercase tracking-[0.12em] text-gray-300"
              ...
              <h3
                className={
                  isDark
                    ? "wrap-break-word text-lg font-bold text-white"
                    : "wrap-break-word text-lg font-bold text-[#280905]"
                }
              >
  ...
  return (
    <aside className="lg:sticky lg:top-24 lg:self-start">
      ...
      <div
        className={
          isDark
            ? "rounded-2xl border border-[#2A2A2A] bg-[#1A1A1A] p-5 shadow-lg shadow-black/10"
...
const EmptyQuoteState = ({ isDark, onBrowse }) => {
  return (
    <div
      className={`flex min-h-screen items-center justify-center px-4 py-20 ${isDark ? "bg-[#090909] text-white" : "bg-[#f7f7f5] text-[rgb(40,9,5)]"}`}
    >
      <Helmet>
        <title>My Quotes | Onasis Links Resources Limited</title>
        <meta name="robots" content="noindex, nofollow" />
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 px-4 backdrop-blur-sm">
      ...
      <div
        className={
          isDark
            ? "w-full max-w-md rounded-2xl border border-[#2A2A2A] bg-[#1A1A1A] p-6 shadow-2xl"
      ...
      >
        ...
        <div className="mt-6 flex flex-col-reverse gap-3 sm:flex-row sm:justify-end">
          <button
            ...
            className={
              isDark
                ? "rounded-xl border border-[#2A2A2A] px-4 py-2.5 text-sm font-semibold text-gray-200 transition hover:bg-[#212121]"
...
const QuoteList = () => {
  ...
  const handleRequestQuote = async () => {
    ...
    try {
      ...
      toast.success(
        `Quote request for ${productCount} product(s) submitted successfully!`,
        {
          duration: 5000,
          position: "bottom-center",
        },
  ...
  if (quoteItems.length === 0) {
    return (
      <EmptyQuoteState isDark={isDark} onBrowse={() => navigate("/products")} />
    );
  }
    ...
    >
      ...
      <div className="mx-auto max-w-350 px-4 py-8 sm:px-6 lg:px-8">
        <div className="mb-8 hidden lg:block">
          <div
            className={`flex items-center gap-3 rounded-2xl border px-4 py-3 shadow-sm backdrop-blur-sm ${isDark ? "border-[#2A2A2A] bg-[#1A1A1A]" : "border-gray-200/70 bg-white/80"}`}
          >
            <CheckCircle2
              className={
                isDark ? "h-5 w-5 text-[#E6501B]" : "h-5 w-5 text-[#C3110C]"
